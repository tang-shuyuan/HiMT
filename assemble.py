...
def simple_gfa(assemble_species,input_file,output_dir,config_path):
    ...
    with open(input_file, "r") as f:
        ...
        if assemble_species=='plant':
            chr_blast_result = os.path.join(output_dir, "blast_output", "chr_blast_result")
            ...
            with open(os.path.join(output_dir, "himt_chloroplast.gfa"), "w") as f_w:
                f.seek(0)
                for line in f:
    ...
    with open(input_file, "r") as f,open(os.path.join(output_dir, "himt_mitochondrial.gfa"), "w") as f_w:
        for line in f:
            if line.startswith("H"):
                f_w.write(line)
            elif line.startswith("S") or line.startswith("L") or line.startswith("P"):
...
def output_two_chl_hap(chl_gfa_file, output):
    ...
    with open(chl_gfa_file, 'r') as f:
        ...
        if len(seq_info) == 1:
            with open(os.path.join(output, "chloroplast_hap1.fa"), 'w') as f_w1:
                f_w1.write(">hap1"+"\n")
            ...
            for line in f:
                if line.startswith("L"):
                    ...
                    if line_content[2] == "+" and line_content[4] == "+" and reversed_repeat in line_content:
                        ...
                        with open(os.path.join(output, "chloroplast_hap2.fa"), 'w') as f_w2:
            ...
            print("your chloroplast genome seems wrong")
...
def assemble(args):
    ...
    total_seq_number,total_seq_len,total_min,total_max,total_estimate_depth=filter(args)

    if args.no_flye_meta:
        os.system(f"flye --pacbio-hifi {os.path.join(args.output_dir, 'extract.fa')} \
            -o {os.path.join(args.output_dir, 'flye_output')} -t {args.thread}")
    else:
        os.system(f"flye --pacbio-hifi {os.path.join(args.output_dir,'extract.fa')} \
    -o {os.path.join(args.output_dir,'flye_output')} -t {args.thread} --meta")

    simple_gfa(args.species,os.path.join(args.output_dir,"flye_output/assembly_graph.gfa"),args.output_dir,config_path)
    ...
    if args.species =="plant":
        ...
        total_max,total_estimate_depth,filter_seq_number, filter_seq_len,\
        ...
        if file_exist(os.path.join(args.output_dir, "himt_chloroplast.gfa")):
            args.table3_value = args.table3_value = [total_seq_number, total_seq_len, total_min, int(round(total_seq_len// total_seq_number,0)),\
            total_max,int(total_estimate_depth/filter_mito_detph*filter_chl_depth),filter_seq_number, filter_seq_len,\
